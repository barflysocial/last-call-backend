...
app.post('/api/sessions/:sessionId/tables/:tableId/join', (req, res) => {
  ...
  const soloNoRole = playMode === 'solo' && !roleId && playerCount === 0;
  ...
  if (playMode === 'solo' && playerCount >= 1) return res.status(409).json({ error: 'Solo Player join is only available on an empty team' });
