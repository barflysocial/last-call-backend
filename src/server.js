...
function getUnlockedEntriesForDetective(truthPackId, elapsedSec) {
  const timeline = buildTimelineForTruthPack(truthPackId);
  const apps = {};
  const publicClues = [];
...
function privatePlayerView(session, table, player) {
  ...
  const roleIds = ROLE_IDS.slice();
  const unlocked = getUnlockedEntriesForDetective(table.truthPackId, elapsedSec);
  const debugAppCounts = Object.fromEntries(Object.entries(unlocked.apps || {}).map(([k,v]) => [k, (v || []).length]));
  ...
  return {
    ...
    apps: unlocked.apps,
    debug: {
      publicClueCount: (unlocked.publicClues || []).length,
      appCounts: debugAppCounts
    }
  };
}
...
app.get('/api/sessions/:sessionId/tables/:tableId/players/:playerId/debug-player', (req, res) => {
  const found = requireTable(req, res);
  if (!found) return;
  const { session, table } = found;
  const player = table.players[req.params.playerId];
  if (!player) return res.status(404).json({ error: 'Player not found' });
  const view = privatePlayerView(session, table, player);
  res.json({
    phase: view.phase,
    elapsedSec: view.elapsedSec,
    player: view.player,
    debug: view.debug,
    publicClues: view.publicClues,
    apps: view.apps
  });
});
...
app.post('/api/sessions/:sessionId/tables/:tableId/help', (req, res) => {
  ...
  if (!Array.isArray(table.helpRequests)) table.helpRequests = [];
