...
const ADAPTIVE_PROFILES = {
  1: [
    { label: 'Solo Investigator', roles: ['bartender','security','host','manager','ex_lover','sound_tech'] }
  ],
  2: [
    { label: 'Lane 1', roles: ['bartender','security','ex_lover'] },
    { label: 'Lane 2', roles: ['host','manager','sound_tech'] },
  ],
  3: [
    { label: 'Lane 1', roles: ['bartender','ex_lover'] },
    { label: 'Lane 2', roles: ['security','manager'] },
    { label: 'Lane 3', roles: ['host','sound_tech'] },
  ],
  4: [
    { label: 'Lane 1', roles: ['bartender','ex_lover'] },
    { label: 'Lane 2', roles: ['security'] },
    { label: 'Lane 3', roles: ['host'] },
    { label: 'Lane 4', roles: ['manager','sound_tech'] },
  ],
  5: [
    { label: 'Lane 1', roles: ['bartender','ex_lover'] },
    { label: 'Lane 2', roles: ['security'] },
    { label: 'Lane 3', roles: ['host'] },
    { label: 'Lane 4', roles: ['manager'] },
    { label: 'Lane 5', roles: ['sound_tech'] },
  ],
  6: [
    { label: 'Bartender', roles: ['bartender'] },
    { label: 'Security', roles: ['security'] },
    { label: 'Karaoke Host', roles: ['host'] },
    { label: 'Manager', roles: ['manager'] },
    { label: 'Ex-Lover', roles: ['ex_lover'] },
    { label: 'Sound Tech', roles: ['sound_tech'] },
  ]
};

function getAdaptiveLane(table, player) {
  const players = Object.values(table.players || {}).sort((a, b) => (a.joinedAt || 0) - (b.joinedAt || 0));
  const count = Math.max(1, Math.min(6, players.length || 1));
  const profiles = ADAPTIVE_PROFILES[count] || ADAPTIVE_PROFILES[6];
  const idx = Math.max(0, players.findIndex(p => p.id === player.id));
  return profiles[Math.min(idx, profiles.length - 1)] || profiles[0];
}
...
function publicTableView(table) {
  return {
    ...
    mode: table.mode || 'team',
    playerTargetCount: (table.mode || 'team') === 'solo' ? 1 : 6,
    ...
    hostMessages: (table.hostMessages || []).slice(-5),
    helpRequests: (table.helpRequests || []).filter(h => !h.resolvedAt),
    timers: {
...
function getUnlockedEntriesForRoles(truthPackId, roleIds, elapsedSec) {
  const timeline = buildTimelineForTruthPack(truthPackId);
  const apps = {};
  const publicClues = [];
  const allow = new Set(roleIds || []);
  ...
  for (const item of timeline) {
    ...
    for (const entry of item.entries || []) {
      if (!allow.has(entry.role)) continue;
...
function privatePlayerView(session, table, player) {
  ...
  const adaptiveLane = (table.mode || 'team') === 'solo' ? ADAPTIVE_PROFILES[1][0] : getAdaptiveLane(table, player);
  const roleIds = player.roleId ? (adaptiveLane?.roles || [player.roleId]) : [];
  const unlocked = getUnlockedEntriesForRoles(table.truthPackId, roleIds, elapsedSec);
  ...
  return {
    ...
    player: {
      ...
      roleLabel: player.roleId ? ROLE_LABELS[player.roleId] : null,
      roleInfo: player.roleId ? getRoleInfo(player.roleId) : null,
      adaptiveRoles: roleIds,
    ...
    hostMessages: (table.hostMessages || []).slice(-10),
    unlocks: unlocked,
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    adaptiveProfiles: Object.keys(ADAPTIVE_PROFILES),
  });
});
...
app.post('/api/sessions/:sessionId/tables/:tableId/rename', (req, res) => {
  ...
  for (const other of Object.values(session.tables || {})) {
    if (other.id === table.id) continue;
    if (normalizeTeamLabel(other.label) === normalized) {
      return res.status(409).json({ error: 'That team name is already in use' });
    }
  }
...
app.post('/api/sessions/:sessionId/tables/:tableId/mode', (req, res) => {
  const found = requireTable(req, res);
  if (!found) return;
  const { session, table } = found;
  const mode = req.body?.mode === 'solo' ? 'solo' : 'team';
  table.mode = mode;
  persistAndBroadcast(session, { type: 'table_updated', table: publicTableView(table) });
  res.json(publicTableView(table));
});
...
app.post('/api/sessions/:sessionId/tables/:tableId/join', (req, res) => {
  ...
  const playMode = req.body?.playMode === 'solo' ? 'solo' : 'multi';

  if (!playerName) return res.status(400).json({ error: 'playerName is required' });
  const soloNoRole = (table.mode || 'team') === 'solo' && playMode === 'solo' && !roleId;
  if (!soloNoRole && !ROLE_IDS.includes(roleId)) return res.status(400).json({ error: 'Invalid roleId' });
  if (!soloNoRole && table.claims[roleId]) return res.status(409).json({ error: 'Role already claimed' });
  if ((table.mode || 'team') === 'solo' && Object.keys(table.players || {}).length >= 1) return res.status(409).json({ error: 'Solo table already has a player' });
...
app.post('/api/sessions/:sessionId/tables/:tableId/players/:playerId/role', (req, res) => {
  const found = requireTable(req, res);
  if (!found) return;
  const { session, table } = found;
  const player = table.players[req.params.playerId];
  ...
  if (table.claims[roleId] && table.claims[roleId].playerId !== player.id) return res.status(409).json({ error: 'Role already claimed' });

  if (player.roleId && table.claims[player.roleId]?.playerId === player.id) {
    delete table.claims[player.roleId];
  }
...
app.post('/api/sessions/:sessionId/tables/:tableId/help', (req, res) => {
  const found = requireTable(req, res);
  if (!found) return;
  const { session, table } = found;
  const reason = String(req.body?.reason || 'Need host assistance').trim() || 'Need host assistance';
  const help = {
    id: randomUUID(),
    reason,
    createdAt: nowMs(),
    teamName: table.label,
    resolvedAt: null
  };
  table.helpRequests.push(help);
  persistAndBroadcast(session, { type: 'table_updated', table: publicTableView(table) });
  broadcastSession(session.id, { type: 'help_request', tableId: table.id, help });
  res.status(201).json(help);
});

app.post('/api/sessions/:sessionId/tables/:tableId/help/:helpId/resolve', (req, res) => {
  const found = requireTable(req, res);
  if (!found) return;
  const { session, table } = found;
  const help = (table.helpRequests || []).find(h => h.id === req.params.helpId);
  if (!help) return res.status(404).json({ error: 'Help request not found' });
  help.resolvedAt = nowMs();
  persistAndBroadcast(session, { type: 'help_resolved', tableId: table.id, helpId: help.id });
  res.json({ ok: true, helpId: help.id });
});

app.post('/api/sessions/:sessionId/tables/:tableId/message', (req, res) => {
  const found = requireTable(req, res);
  if (!found) return;
  const { session, table } = found;
  ...
  table.hostMessages.push(message);
  table.hostMessages = table.hostMessages.slice(-20);
  persistAndBroadcast(session, { type: 'team_message', tableId: table.id, message });
  res.status(201).json(message);
});

app.post('/api/sessions/:sessionId/message-all', (req, res) => {
  const session = requireSession(req, res);
  if (!session) return;
  const messageText = String(req.body?.message || '').trim();
  if (!messageText) return res.status(400).json({ error: 'message is required' });
  ...
  for (const table of Object.values(session.tables || {})) {
    ...
    if (!Array.isArray(table.hostMessages)) table.hostMessages = [];
    table.hostMessages.push(message);
    table.hostMessages = table.hostMessages.slice(-20);
    payloads.push({ tableId: table.id, message });
  }
  persistSession(session);
  for (const item of payloads) {
    broadcastSession(session.id, { type: 'broadcast_message', tableId: item.tableId, message: item.message });
  }
  res.status(201).json({ ok: true, sent: payloads.length });
});
